...
(function() {
  var Game, atom, c, cancelAnimationFrame, determineGesture, eventCode, requestAnimationFrame, _i, _ref, _ref1,
  ...
  determineGesture = function(gesture) {
    var dx, dy, end, start;
    if (gesture.length === 1) {
    ...
    if (Math.abs(dx) > Math.abs(dy)) {
      if (dx > 0) {
        return atom.touch.SWIPE_RIGHT;
      } else {
        return atom.touch.SWIPE_LEFT;
      }
    } else {
      if (dy > 0) {
        return atom.touch.SWIPE_DOWN;
      } else {
        return atom.touch.SWIPE_UP;
      }
    }
  };
  ...
  atom.setDesiredSurfaceArea = function(desiredSurfaceArea) {
    atom.desiredSurfaceArea = desiredSurfaceArea;
    return window.onresize();
  };

  window.onresize = function(e) {
    var actualSurfaceArea, upscale;
    upscale = 1;
